...
%convex
[C, tau, error, X_new] = find_tau(X_noise,tau_range,'noise convex',num,c);
tau
figure;
scatter(1:n*c,C);
for i = 1:c*n
...
%non-convex
[C, tau, error, X_new] = find_tau(X_noise,tau_range,'noise nonconvex',num,c);
tau
figure;
scatter(1:n*c,C);
...
%convex
[C, tau, error, X_new] = find_tau(X_noise,tau_range,'corrupt convex',num,c);
tau
figure;
scatter(1:n*c,C);

%non-convex
[C, tau, error, X_new] = find_tau(X_noise,tau_range,'corrupt nonconvex',num,c);
tau
figure;
scatter(1:n*c,C);

%% LRSC with Uncorrupted Entries
[C, tau, error, X_new] = find_tau(X_noise,tau_range,'uncorrupt',num,c);
tau
figure;
scatter(1:n*c,C);
